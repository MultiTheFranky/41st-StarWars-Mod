...
if ((TF_tangent_lr_pressed) and {alive TFAR_currentUnit}) then {
	...
	if (typeOf (_radio select 0) == "Land_HelipadEmpty_F") then
	{
		...
		"\SWOP_clones\data\helmets\ico\ico_arf.paa"],(_radio call TFAR_fnc_getLrChannel) + 1, call TFAR_fnc_currentLRFrequency],
